...
namespace MaxsMusicQuiz.Backend.Repositories
{
    public class LeaderboardRepository(ApplicationDbContext dbContext) : ILeaderboardRepository
    {
        ...
        {
            // First, get the users without the index calculation
            ...
            // Then manually create the DTOs with rank calculation
        ...
        {
            // First get the users ordered by game history count
            var users = await dbContext.Users
            ...
            // Then manually create DTOs with the rank
            return users.Select((u, index) => new LeaderboardEntryDto
            {
                Rank = offset + index + 1,
                UserId = u.Id,
